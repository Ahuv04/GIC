...
class SocketCommunication(Node):
    ...
    def node_message(self, connected_node, message):
        ...
        # Ownership : done
        # verification : done 
        # reply on verification : 
        elif message.messageType == 'OWNERSHIP':
            transaction = message.data
            self.node.handleTransaction(transaction)
        elif message.messageType == 'VERIFICATION':
            transaction = message.data
            self.node.verificationRequest(connected_node,transaction)
        elif message.messageType == 'VERIFREPLY':
            print(connected_node.port)
            transaction = message.data
            self.node.verifReply(transaction)
