from Wallet import Wallet
from BlockchainUtils import BlockchainUtils
import requests

if __name__ == '__main__':

    bob = Wallet()
    alice = Wallet()
    exchange = Wallet()
    stakerPublicKey = "-----BEGIN PUBLIC KEY-----\nMIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCz5GsVfnrZJdX+121CA8e2pa0s\nSDPIgAkWoruT29I14NYPnK77BwIxTxUFfa68BT5uLCqmrG3Et8mfSuNNhdhrJJOZ\nZVdKll5JEomvpLCmImbNPyWxjTA75w7R7cUeLL6oJh7UChKKrZs+T2/7WqpmGw7u\nO0OFWWc8/Eh3MD7ISwIDAQAB\n-----END PUBLIC KEY-----"
    node1PublicKey = "-----BEGIN PUBLIC KEY-----\\n9wIDAQAB\n-----END PUBLIC KEY-----"
    node2PublicKey = "-----BEGIN PUBLIC KEY-----\\nNQIDAQAB\n-----END PUBLIC KEY-----"

    # parts = {}
    # author = ''
    # vin = ''
    # ttype = 'EXCHANGE'
    # coins = 1000
    # recieverKey = stakerPublicKey

    # url = "http://localhost:5001/transaction"
    # package = {"type": ttype,"coins": coins, "parts":parts, "ownerID":author,"vin":vin, "recieverKey":recieverKey}
    # request = requests.post(url, json=package)
    # print(request.text)

    # parts = {}
    # author = ''
    # vin = ''
    # ttype = 'STAKE'
    # coins = 100
    # recieverKey = stakerPublicKey

    # url = "http://localhost:5002/transaction"
    # package = {"type": ttype,"coins": coins, "parts":parts, "ownerID":author,"vin":vin, "recieverKey":recieverKey}
    # request = requests.post(url, json=package)
    # print(request.text)

    # parts = {'Engine': 150, 'PartB': 200, 'PartC':700}
    # ownerID = 'Abhay'
    # vin = 'VIN0001'
    # ttype = 'OWNERSHIP'
    # coins = 200
    # recieverKey = node1PublicKey

    # url = "http://localhost:5002/transaction"
    # package = {"type": ttype,"coins": coins, "parts":parts, "ownerID":ownerID,"vin":vin, "recieverKey":recieverKey}
    # request = requests.post(url, json=package)
    # print(request.text)

    
    parts = {'Engine': 150, 'PartB': 200, 'PartC':700}
    ownerID = 'Abhay'
    vin = 'VIN0001'

    url = "http://localhost:5002/verifyParts"
    package = {"parts":parts, "ownerID":ownerID,"vin":vin}
    request = requests.post(url, json=package)
    print(request.text)