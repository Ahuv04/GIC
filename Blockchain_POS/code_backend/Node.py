...
class Node():
    ...
    def __init__(self, ip, port, key=None, passPhrase=None):
        self.p2p = None
        self.ip = ip
        self.port = port
        ...
        self.wallet = Wallet()
        if key is not None:
            self.wallet.fromKey(key, passPhrase)
    ...
    def handleBlock(self, block):
        ...
        sameBlockAdding = self.blockchain.blocks[-1].blockCount == block.blockCount
        if not blockCountValid:
            self.requestChain()
        if lastBlockHashValid and forgerValid and transactionsValid and signatureValid and not sameBlockAdding:
            print("adding block in handle block")
        ...
        if localBlockCount < receivedChainBlockCount:
            for blockNumber, block in enumerate(blockchain.blocks):
                if blockNumber >= localBlockCount:
                    print("adding block in handle blockchain")
